...
function createTablaUsuarios()
{
    try {
        ...
        else
        {
            ...
            $sql = 'CREATE TABLE `usuarios` (`id` INT NOT NULL AUTO_INCREMENT , `username` VARCHAR(50) NOT NULL UNIQUE , `nombre` VARCHAR(50) NOT NULL , `apellidos` VARCHAR(100) NOT NULL , `contrasena` VARCHAR(100) NOT NULL , `rol` INT NOT NULL DEFAULT 0, PRIMARY KEY (`id`)) ';
            if ($conexion->query($sql))
            {
...
function CrearTablaFicheros() {
    try {
        ...
        if ($conexion->connect_error) {
            ...
            if ($conexion->query($sql)) {
                return [true, 'Tabla "ficheros" creada exitosamente.'];
            } else {
                return [false, 'Error al crear la tabla: ' . $conexion->error];
            }
        }
    } catch (mysqli_sql_exception $e) {
