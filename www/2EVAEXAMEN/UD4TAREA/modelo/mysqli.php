...
function conectaTareas()
{
    ...
    return conecta($hostname, $username, $password, $database);
}
...
function createTablaUsuarios()
{
    try {
        ...
        else
        {
            ...
            $sql = 'CREATE TABLE `usuarios` (`id` INT NOT NULL AUTO_INCREMENT , `username` VARCHAR(50) NOT NULL UNIQUE , `nombre` VARCHAR(50) NOT NULL , `apellidos` VARCHAR(100) NOT NULL , `contrasena` VARCHAR(100) NOT NULL , PRIMARY KEY (`id`)) ';
            if ($conexion->query($sql))
            {
...
function createTablaFichero(){
    try{
        ...
        if($conexion->connect_error){
            ...
            $sql='CREATE TABLE  IF NOT EXISTS `ficheros` (`id` INT NOT NULL AUTO_INCREMENT, `nombre`VARCHAR(100) NOT NULL, `file`VARCHAR(250) NOT NULL, `descripcion` VARCHAR(250) NOT NULL,`id_tarea`INT NOT NULL ,PRIMARY KEY(`id`),FOREIGN KEY (`id_tarea`)REFERENCES `tareas`(`id`))';
            if($conexion->query($sql))
            {
...
function buscaTarea($id)
{
    ...
    else
    {
        ...
        if ($resultados->num_rows == 1)
        {
            ...
            $usuario=buscaUsuarioMysqli($row['id_usuario']);
            //from para obtener la instancia correspondiente de estado.
...
function buscaUsuarioMysqli($id)
{
    ...
    else
    {
        ...
        if ($resultados->num_rows == 1)
        {   
            //fetch seignifica obtener o traer
            $row=$resultados->fetch_assoc();
            $usuario=new Usuario();
